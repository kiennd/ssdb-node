...
class Connection {
    ...
    connect(callback) {
        ...
        this.sock.on('error', function (err) {
            ...
            if (self.parent._promisify && self.callbacks.length > 0){
                let callback = self.callbacks.shift();
                self.commands.shift();
                callback(error, undefined);
            }

            if (self.parent != null && self.parent.options.autoReconnect){
                ...
                if (index >= 0){
                    ...
                    index = self.parent._idleConn.indexOf(self);
                    if (index >= 0) {
                        self.parent._idleConn.splice(index, 1);
                        needReconn = true;
                    }
                }
                if (needReconn) {
                    self.parent._reconnectCount++;
                    let reconnectWait = Math.min(Math.pow(2, parseInt((self.parent._reconnectCount - 1) / Math.max(self.parent.options.size, 1))) * 1000, 120000);
                    self.log.info(`Reconnect to ${self.host}:${self.port} after ${reconnectWait / 1000} seconds...`);
                    setTimeout((self)=>{
                        self.parent._invalidConn.push(self);
    ...
    onReceived(buf) {
        ...
        responses.forEach(function (response) {
            ...
            if (self.callbacks.length !== 0){
                console.error("ssdb client Error, please report to author!")
            }
            if (self.parent != null){
                let index = self.parent._busyConn.indexOf(self);
                if (index >= 0){
    ...
    request(cmd, params, callback) {
        if (["qpop_front", "qpop_back"].indexOf(cmd) >= 0 && params.length > 1 && params[1] > 1){
            this.commands.unshift("__" + cmd);
        }
        else {
...
class Client {
    constructor(options) {
        ...
        options.autoReconnect = options.autoReconnect === undefined ? true : !!options.autoReconnect;
        options.parent = this;
        this.options = options;
        for (let i = 0; i < (this.options.size || 1); ++i) {
            this._idleConn.push(new Connection(this.options))
        }
    ...
    _checkResume(){
        while (this._invalidConn.length > 0){
            try {
                let conn = this._invalidConn.shift();
                conn.close();
            }
            catch (e){
                console.error(e);
    ...
    close() {
        this._waitSend = [];
        for (let conn of this._busyConn){
            conn.close();
        }
        for (let conn of this._idleConn){
            conn.close();
        }
    };
}
